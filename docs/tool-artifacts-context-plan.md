...
# برنامهٔ artifactهای خروجی ابزار / پُشدن زمینه

تاریخ: ۲۰۲۶-۰۴-۲۴

## مسئله

نشست‌های Hermes Agent در جریان کارهای معمول مبتنی بر ابزار، زمینه را به‌سرعت پر می‌کنند. در یک نشست فضای کار (`cda915a9-fbf6-4bbf-9617-e7e9d26f40be`):

- مجموعاً ۲۵۶ پیام
- ۷ پیام کاربر
- ۱۱۷ پیام دستیار
- ۱۳۲ پیام ابزار
- حدود ۶۰۳k کاراکتر خروجی ابزار
- حدود ۱۵۲k token تقریبی

پُشدن، از گفتگوی کاربر/دستیار نیست. بیشتر رد اجرای ذخیره‌شده است:
...
۳. رابط کاربری گفتگوی فضای کار توسط کارت‌های ابزار تسلط می‌یابد و به‌نظر می‌رسد پیام‌های کاربر/دستیار ناپدید شده‌اند.
۴. مدل در هر نوبت خروجی‌های بزرگ ابزار را به‌اجبار دریافت می‌کند حتی وقتی فقط به pointer/خلاصه‌ها نیاز دارد.
...
پس پنل سمت راست مقصد UX درست است، ولی به یک backing store دوام‌دار نیاز دارد.

## گزینه‌های پیاده‌سازی

### گزینهٔ A — artifact store رسمی Hermes Agent

بهترین بلندمدت، زیرا همهٔ کلاینت‌ها سود می‌برند: CLI، دروازه، فضای کار، WebUI آینده.
...
### گزینهٔ B — cache artifact MVP فقط فضای کار

patch سریع‌تر فقط فضای کار.
...
فضای کار `/api/history` یا `/api/send-stream` خروجی‌های ابزار بیش‌ازحد بزرگ را قبل از رندر/ارسال به React بیرونی می‌کند.

MVP خوبی است، ولی برای پُشدن زمینهٔ مدل رسمی کافی نیست مگر اینکه context builder Hermes Agent نیز تزریق مجدد payload کامل ابزار را متوقف کند.
...
## رفع‌های رابط کاربری فضای کار آغازشده
...
- `src/screens/chat/components/chat-message-list.test.tsx`
  ...
  - regression برای شناسایی دم فقط-ابزار پنهان تا فضای کار بتواند یک وضعیت پایانی قابل‌فهم برای انسان رندر کند

## artifact store MVP فضای کار پیاده‌سازی شد
...
- `src/routes/api/history.ts`
  ...
  - همان مسیر بیرونی‌سازی را به پیام‌های fallback نشست portable محلی اعمال می‌کند
...
۳. **راه‌اندازی/نشست فعلی ما آن را تقویت می‌کند** زیرا این گفتگو به‌شدت مبتنی بر ابزار است و نزدیک حداکثر زمینه است؛ اقدامات dev/ابزار پس از یک پاسخ دستیار قابل‌فهم برای انسان می‌توانند ردیف‌های ابزار ذخیره‌شده در دم به‌جا بگذارند.

نتیجه: فضای کار باید این وضعیت را به‌صورت دفاعی جمع/خلاصه-ابزار کند، ولی رفع رسمی در stream/session writer Hermes Agent تعلق دارد: هرگز یک نوبت گفتگوی تکمیل‌شده را که فقط با خروجی ابزار خاتمه می‌یابد، ذخیره نکنید.
...
- `run_agent.py`
  - `_persist_session()` اکنون قبل از نوشتن JSON/SQLite، `_ensure_terminal_assistant_message()` را صدا می‌زند.
...
۱. fallback فضای کار را نگه دارید، ولی آن را به‌عنوان مدیریت وضعیت لبه در نظر بگیرید.
۲. کار artifact رسمی را در Hermes Agent ادامه دهید: خروجی‌های ابزار بیش‌ازحد بزرگ را قبل از آنکه وارد زمینهٔ ذخیره‌شده/مدل شوند، بیرونی کنید، نه فقط قبل از آنکه فضای کار تاریخچه را رندر کند.
۳. ادامهٔ پولیش فضای کار:
