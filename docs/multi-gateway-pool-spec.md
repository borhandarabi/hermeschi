...
HermesChi در حال حاضر به‌عنوان یک **رابط کاربری تک‌دروازه‌ای، تک‌پروفایلی** عمل می‌کند. دروازه یک `HERMES_HOME` را در زمان شروع بارگذاری می‌کند و همهٔ نشست‌های گفتگو، عملیات و دسترسی memory از همان یک فرآیند عبور می‌کنند.
...
- **بدون هویت profile در گفتگو**: «عامل» همیشه همان کسی است که دروازهٔ تک‌پروفایلی به‌عنوان او راه‌اندازی شده
...
- **آلودگی نشست**: همهٔ نشست‌ها فارغ از اینکه کدام شخصیت عامل آن‌ها را ایجاد کرده، در یک استخر انباشته می‌شوند

### داستان کاربر

> «من با Nous به‌طور راهبردی دربارهٔ ویژگی‌های Ascent Performance فکر می‌کنم، در حالی که Jules ساختن با Architect و Sentinel را هماهنگ می‌کند. می‌خواهم در همان پنجرهٔ فضای کار به‌سرعت میان این گفتگوهای عامل تعویض کنم، با هر عامل که memory، skills و زمینهٔ نشست خود را حفظ می‌کند.»
...
**نتیجه**: فضای کار باید یک **هماهنگ‌کنندهٔ عامل** باشد، نه فقط یک پوستهٔ رابط کاربری روی یک دروازه.
...
**راه‌حل**: فضای کار یک **استخر دروازه** را نگه می‌دارد — یک فرآیند دروازه به‌ازای هر profile فعال، هر کدام روی پورت خود، همگی تحت نظارت سلامت، همگی قابل مسیریابی از رابط کاربری.

**اصول طراحی:**

۱. **مستقل از تعداد profile**: برای ۱ profile یا ۱۰۰ کار می‌کند. بدون محدودیت‌های hardcoded، آرایه‌ها، یا دستورات switch که profileهای خاص را برمی‌شمرند.
...
۴. **fail-safe**: یک دروازهٔ مرده فضای کار را crash نمی‌کند. fallback ایمن همیشه در دسترس است.
...
**مستقل از تعداد profile**: مدیر استخر profileها را به‌صورت پویا از فایل‌سیستم (`~/.hermes/profiles/*`) کشف می‌کند. هیچ فهرست hardcodedای، هیچ تعداد حداکثری، و هیچ special-casing نام profile خاصی وجود ندارد. کاربری با ۲ profile و کاربری با ۵۰ profile از دقیقاً همان مسیر کد استفاده می‌کنند.
...
**یادداشت**: دروازه باید از طریق `hermes gateway` spawn شود، نه از طریق gateway.ts داخلی فضای کار. فضای کار به یک هماهنگ‌کننده تبدیل می‌شود، نه یک میزبان دروازه.
...
تمام مسیرهای API فضای کار به **زمینهٔ profile** مجهز می‌شوند:
...
در حال حاضر: همهٔ نشست‌ها در `~/.hermes/sessions/` (یا دایرکتوری نشست‌های profile)

با چند-دروازه: هر دروازه نشست‌های خود را در دایرکتوری profile خود مدیریت می‌کند. فضای کار آن‌ها را برای نمایش **تجمیع** می‌کند ولی به‌ازای هر profile **مسیریابی** می‌کند.
...
نشست‌ها در sidebar **بر اساس profile گروه‌بندی** می‌شوند با تمایز بصری:
...
- حالت خالی نام profile فعال + مدل را نشان می‌دهد (همین حالا در PR #118 پیاده‌سازی شده)
- فهرست نشست نقطه‌های رنگی profile را به‌ازای هر نشست نشان می‌دهد
- composer به دروازهٔ profile فعال ارسال می‌کند
- «New Session» نشست معکوب به profile فعال ایجاد می‌کند
...
src/server/sessions-aggregator.ts   # تجمیع نشست چندپروفایلی
...
src/screens/chat/components/chat-empty-state.tsx  # همین حالا انجام شده
src/components/workspace-shell.tsx  # افزودن انتخابگر profile
src/server/local-provider-discovery.ts  # کشف ارائه‌دهنده چند-دروازه‌ای
```
...
| پورت در حال حاضر در استفاده | افزایش خودکار پورت، لاگ هشدار |
| حذف profile در حین اجرای دروازه | توقف دروازه، حذف از استخر |
| crash شدن فضای کار | در restart، بررسی دروازه‌های orphan، adopt یا kill |
...
- دروازه‌ها فقط به `127.0.0.1` bind می‌شوند (همین حالا پیش‌فرض)
- بدون نشت memory بین‌پروفایلی (هر دروازه `HERMES_HOME` خود را دارد)
- انتخابگر profile به middleware احراز هویت احترام می‌گذارد
...
- **بدون مسیرهای hardcoded**: تخصیص پورت، دایرکتوری‌های profile و URLهای دروازه به‌صورت پویا resolve می‌شوند. هیچ مسیر `/Users/...` یا `C:\Users\...` در کد نیست.
...
- **نشست‌های موجود**: حفظ می‌شوند. هر نشست همین حالا در دایرکتوری profile خود زندگی می‌کند. فضای کار فقط آن‌ها را به‌درستی تجمیع می‌کند.
...
۲. **فاز ۲ (پیگیری)**: تجمیع نشست با گروه‌بندی profile
...
- Issue #?: مدیریت نشست چندپروفایلی (برای ایجاد)
...
۱. آیا فضای کار باید همهٔ دروازه‌های profile را در startup به‌صورت خودکار spawn کند، یا فقط در نخستین استفاده؟
۲. آیا باید یک profile «پیش‌فرض فضای کار» همیشه فعال باشد، یا هر پنل باید آخرین profile خود را به یاد بیاورد؟
...
*معماری بر اساس اصول اولیه: اگر هر profile یک عامل متمایز است، فضای کار باید یک هماهنگ‌کنندهٔ عامل باشد.*
