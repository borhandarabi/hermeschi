...
const DIALOG_LINE_KEYS = [
  'playground.questDialog.dialogLine1',
  'playground.questDialog.dialogLine2',
  'playground.questDialog.dialogLine3',
] as const

const CHOICES: QuestChoice[] = [
  { id: 'ask-lore', label: t('playground.questDialog.choiceAsk'), tone: t('playground.questDialog.toneLore') },
  { id: 'promise', label: t('playground.questDialog.choicePromise'), tone: t('playground.questDialog.toneHeroic') },
  { id: 'request-reward', label: t('playground.questDialog.choiceReward'), tone: t('playground.questDialog.tonePragmatic') },
]

export function QuestDialogPanel({ npcName = t('playground.questDialog.npcAthena'), npcTitle = t('playground.questDialog.npcTitleAthena'), onAccept, onDecline }: QuestDialogPanelProps) {
  return (
    <section
      role="dialog"
      aria-label={t('playground.questDialog.ariaLabel')}
      ...
      <div className="grid gap-0 md:grid-cols-[230px_1fr]">
        <aside className="border-b border-[#d9b35f]/25 bg-[radial-gradient(circle_at_50%_15%,rgba(251,191,36,.24),transparent_46%),linear-gradient(180deg,rgba(217,179,95,.16),rgba(0,0,0,.18))] p-5 md:border-b-0 md:border-r">
          <div className="mx-auto flex max-w-[190px] flex-col items-center text-center">
            <div className="relative h-36 w-36 overflow-hidden rounded-[28px] border-2 border-[#d9b35f]/55 bg-[linear-gradient(180deg,rgba(217,179,95,.24),rgba(17,24,39,.9))] shadow-[0_0_35px_rgba(217,179,95,.22)]">
              <img
                alt={t('playground.questDialog.portraitAlt', { name: npcName })}
            ...
            <h2 className="mt-4 font-serif text-2xl font-black text-[#ffe7a3]">{t('playground.questDialog.header')}</h2>
        ...
        <div className="flex min-h-[430px] flex-col p-5">
          <div className="mb-3 flex items-center justify-between gap-3 border-b border-[#d9b35f]/20 pb-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#d9b35f]/70">{t('playground.questDialog.mainQuest')}</p>
              <h3 className="font-serif text-xl font-black text-[#ffe7a3]">{t('playground.questDialog.title')}</h3>
            </div>
            <span className="rounded-full border border-emerald-300/35 bg-emerald-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-emerald-200">{t('playground.questDialog.available')}</span>
          </div>

          <div aria-label={t('playground.questDialog.scrollAria')} role="region" className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-[#d9b35f]/18 bg-black/28 p-4 shadow-inner">
            {DIALOG_LINE_KEYS.map((key) => (
              <p key={key} className="mb-3 text-sm leading-6 text-[#f9e7b5]/84">
                <span className="mr-2 text-[#d9b35f]">✦</span>{t(key)}
              </p>
            ))}
          </div>
          ...
          <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button onClick={onDecline} className="rounded-2xl border border-white/14 px-4 py-2 text-sm font-bold text-white/65 transition hover:bg-white/8">
              {t('playground.questDialog.decline')}
            </button>
            <button onClick={onAccept} className="rounded-2xl border border-[#fbbf24]/50 bg-[linear-gradient(180deg,#fde68a,#d9b35f)] px-5 py-2 text-sm font-black text-[#1d1307] shadow-[0_12px_35px_rgba(217,179,95,.28)] transition hover:brightness-110">
              {t('playground.questDialog.accept')}
            </button>
          </div>
        </div>
      </div>
