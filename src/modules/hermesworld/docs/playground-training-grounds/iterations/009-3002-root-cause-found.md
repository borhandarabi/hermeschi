...
**Three concurrent `vite dev` processes were running against the same `hermeschi` source tree.**

Process inventory at start of session:
- pid 40934 — vite dev on **:3001** (running 36+ min, cwd `/Users/aurora/hermeschi`)
...
All three duplicate `hermeschi` vite servers were running **TanStack Router's
...
1. Killed the three duplicate hermeschi vite processes.
...
If more than one vite is bound to the same source tree (`/Users/aurora/hermeschi`),
that is the bug. Worktree vites at different cwds are fine.

A single command that detects it:

```bash
ps aux | grep -E "vite.*hermeschi/node_modules" | grep -v grep | grep -v worktree | wc -l
```
