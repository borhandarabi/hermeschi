...
import { HERMESCHIWORLD_SETTINGS_KEY, loadHermesChiWorldSettings, saveHermesChiWorldSettings } from './hermeschiworld-settings'

const WARNING_DISMISSED_KEY = 'hermeschiworld:photosensitive-warning-dismissed'
...
export function PhotosensitiveWarningSplash({ onOpenSettings }: Props) {
  ...
  const enable = () => {
    const current = loadHermesChiWorldSettings()
    ...
    try {
      ...
      if (raw) window.localStorage.setItem(HERMESCHIWORLD_SETTINGS_KEY, raw)
    } catch {}
    dismiss()
    onOpenSettings?.()
  }
