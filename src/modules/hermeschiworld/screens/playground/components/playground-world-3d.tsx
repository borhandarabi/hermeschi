...
import { useHermesChiWorldSettings } from './hermeschiworld-settings'
...
function npcGlbUrl(id: string) {
  ...
  return `/assets/hermeschiworld/characters/${safe}.glb`
}
...
const ZONE_FALLBACK_BACKGROUNDS: Partial<Record<PlaygroundWorldId, string>> = {
  training: '/assets/hermeschiworld/zones/zone-1.jpg',
  forge: '/assets/hermeschiworld/zones/zone-2.jpg',
  grove: '/assets/hermeschiworld/zones/zone-3.jpg',
  oracle: '/assets/hermeschiworld/zones/zone-4.jpg',
  arena: '/assets/hermeschiworld/zones/zone-5.jpg',
}
...
function TechPillars({ world }: { world: WorldDef }) {
  const [settings] = useHermesChiWorldSettings()
    ...
    <group ref={ref} position={position} onPointerDown={(e) => {
      ...
      {ambient && !isNear && (
        <Html position={[0, 2.7, 0]} center distanceFactor={8}>
          <SpeechBubble className="hermeschi-world-bubble" variant="npc" accent={color} compact>
            {ambient}
          </SpeechBubble>
        </Html>
      )}
...
function PlayerAndCamera({
  ...
  useEffect(() => {
    const tryJump = () => {
      ...
      try { window.dispatchEvent(new CustomEvent('hermeschiworld-player-jumped')) } catch {}
    }
    const onDash = () => {
    ...
    window.addEventListener('hermeschiworld-mobile-jump', onJump)
    window.addEventListener('hermeschiworld-mobile-crouch', onCrouch)
    return () => {
      window.removeEventListener('hermes-playground-dash', onDash)
      window.removeEventListener('hermeschiworld-mobile-jump', onJump)
      window.removeEventListener('hermeschiworld-mobile-crouch', onCrouch)
    }
  }, [])
...
function SelfChatBubble() {
  ...
  return (
    <Html position={[0, 2.85, 0]} center distanceFactor={8}>
      <SpeechBubble className="hermeschi-world-bubble" variant="player" compact>
        {bubble.text}
      </SpeechBubble>
    </Html>
  )
    ...
    <group ref={ref} position={bot.spawn}>
      ...
      {bubbleText && (
        <Html position={[0, 2.6, 0]} center distanceFactor={8}>
          <SpeechBubble className="hermeschi-world-bubble" variant="party" accent={bot.color} compact>
            {bubbleText}
          </SpeechBubble>
        </Html>
      )}
  ...
  return (
    <group ref={ref} position={[remote.x, remote.y, remote.z]}>
      ...
      {remote.lastChat && remote.lastChatAt && Date.now() - remote.lastChatAt < 5500 && (
        <Html position={[0, 2.6, 0]} center distanceFactor={8}>
          <SpeechBubble className="hermeschi-world-bubble" variant="party" accent={remote.color} compact>
            {remote.lastChat}
          </SpeechBubble>
        </Html>
      )}
...
function DevFpsSampler() {
  ...
  useFrame(({ clock }, delta) => {
    ...
    console.table([{ scope: 'HermesChiWorld playground', avgFps: Number((1000 / Math.max(1, avgFrame)).toFixed(1)), avgFrameMs: Number(avgFrame.toFixed(2)), p95FrameMsApprox: Number(stats.max.toFixed(2)), jsHeapMb: stats.heap ? Number(stats.heap.toFixed(1)) : 'n/a', samples: stats.frames }])
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        .hermeschiworld-photosensitive .hermes-target-flash { animation: none !important; opacity: 0.82 !important; }
      `}</style>
      <Canvas
        shadows
