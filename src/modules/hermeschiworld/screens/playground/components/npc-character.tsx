...
import { HERMESCHIWORLD_CHARACTER_ARCHETYPES } from '../lib/character-config'
...
export function NpcCharacter({ archetypeId, accent, ...props }: NpcCharacterProps) {
  const archetype =
    HERMESCHIWORLD_CHARACTER_ARCHETYPES.find((entry) => entry.id === archetypeId) ??
    HERMESCHIWORLD_CHARACTER_ARCHETYPES[0]
