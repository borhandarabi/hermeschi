...
import { useHermesChiWorldSettings } from './components/hermeschiworld-settings'
...
export function PlaygroundScreen() {
  ...
  useEffect(() => {
    if (activeQuest?.id === 'training-q1' && rpg.state.playerProfile.questProgress['training-q1'].completedObjectives.length > 0 && !rpg.state.completedQuests.includes('training-q1')) {
      ...
      window.addEventListener('hermeschiworld-player-jumped', onJump, { once: true })
      return () => { window.clearTimeout(id); window.removeEventListener('hermeschiworld-player-jumped', onJump) }
    }
...
function MobileAbilityControls() {
  ...
  const emitCrouch = (active: boolean) => {
    setCrouching(active)
    try { window.dispatchEvent(new CustomEvent('hermeschiworld-mobile-crouch', { detail: { active } })) } catch {}
  }
  const jump = () => {
    try { window.dispatchEvent(new CustomEvent('hermeschiworld-mobile-jump')) } catch {}
  }
  return (
    <>
      <button
      ...
      >
        ...
        <div className="relative h-[400px] overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
            ...
            <img
              src="/assets/hermeschiworld/art/hermeschiworld-logo-horizontal.svg"
              alt="HermesChiWorld"
              width={760}
              height={228}
              fetchPriority="high"
...
function PlaygroundUtilityDock({
  ...
  const captureScreenshot = () => {
    ...
    try {
      ...
      a.href = dataUrl
      a.download = `hermeschiworld-${new Date().toISOString().replace(/[:.]/g, '-')}.png`
      a.click()
    } catch {}
  }
