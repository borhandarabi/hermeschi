#!/usr/bin/env bash
# scripts/ascii-trailer.sh — turn a screen recording of HermesChiWorld into ASCII frames
...
#
# Usage:
#   ./scripts/ascii-trailer.sh ~/Movies/hermeschiworld-demo.mov
set -euo pipefail
INPUT="${1:-}"
if [[ -z "$INPUT" || ! -f "$INPUT" ]]; then
  echo "Usage: $0 <video-file>"
  echo "Example: $0 ~/Movies/hermeschiworld-demo.mov"
  exit 1
fi
...
{
  echo "# HermesChiWorld — ASCII Trailer"
  echo ""
