...
function getReservedNames(): Set<string> {
  const extra = (process.env.HERMESCHIWORLD_RESERVED_NAMES || '')
    .split(',')
...
export async function createReservation(
  ...
  const baseUrl = (options.baseUrl || process.env.HERMESCHIWORLD_RESERVE_BASE_URL || 'https://hermeschiworld.ir').replace(/\/$/, '')
...
async function supabaseRequest(path: string, init: RequestInit): Promise<Response> {
  const url = requireEnv('HERMESCHIWORLD_SUPABASE_URL').replace(/\/$/, '')
  const key = requireEnv('HERMESCHIWORLD_SUPABASE_SERVICE_ROLE_KEY')
...
export async function sendReservationConfirmationEmail(
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: [payload.email],
      subject: `Confirm your HermesChiWorld name reservation: ${payload.desiredName}`,
      html: `<div style="font-family:Inter,Arial,sans-serif;line-height:1.6;color:#111827">
  <h2>Confirm your HermesChiWorld reservation</h2>
