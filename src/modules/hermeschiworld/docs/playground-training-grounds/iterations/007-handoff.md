...
Latest commits:
- `215bdf8a4` feat(chat): HermesChiWorld featured nav slot + auto-hide sidebar in playground
- `8312ba810` feat(chat): rebrand Playground nav item to HermesChiWorld with gold NEW badge
...
- HermesChiWorld is now a **promoted link directly under Search → New Session**.
...
- `/playground` now **auto-hides desktop sidebar** and also hides `ChatPanel` + `ChatPanelToggle`.
  - Reason: HermesChiWorld HUD uses many `position: fixed` overlays, and the sidebar/chat panel were visually overlapping the game surface.

### HermesChiWorld polish
- 3 new avatar presets: **Chronos / Artemis / Eros**.
- Cinematic camera mode: **Tab** cycles 6 filming angles.
- Camera preset toast added.
- Transition loading screen upgraded to gold HermesChiWorld lore card.
...
- HermesChiWorld feels much more branded and demo-ready.
...
- This is **not just HermesChiWorld**. It includes earlier local-only work:
...
- Do **not** casually push this branch straight to main without deciding whether to:
  ...
  2. cherry-pick only HermesChiWorld commits to a clean branch.
...
- Evaluate whether HermesChiWorld needs a small **"Back to Workspace"** button since the sidebar is now hidden on the route.
- Possibly replace the plain Castle icon with a **custom tiny HermesChiWorld lockup/icon** if Eric wants something more branded than a stock icon.
- Tune the top title/page title from `Playground` to `HermesChiWorld` anywhere still visible in browser/UI.
- Check if any remaining copy still says **Playground** instead of HermesChiWorld.

### If preparing to ship
- Record demo first.
- Decide PR strategy:
  - integration PR from current branch, or
  - clean HermesChiWorld-only branch via cherry-pick.
...
- Demo script:
  - `memory/goals/2026-05-03-playground-training-grounds/HERMESCHIWORLD-FINAL-DEMO-SCRIPT.md`
- Ship checklist:
...
3. Confirm the promoted HermesChiWorld link looks right in the left nav.
