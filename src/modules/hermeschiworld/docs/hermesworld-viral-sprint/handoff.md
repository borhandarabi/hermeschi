# HermesChiWorld Viral Sprint Handoff
...
### 1. HermesChiWorld landing was rebuilt into a standalone public surface
Routes in repo:
- `/hermeschi-world`
- `/world`

Key file:
- `src/screens/playground/hermeschi-world-landing.tsx`
...
### 4. Bottom-right usage/session pill leak on HermesChiWorld/game surfaces was addressed
...
### 6. HermesChiWorld landing hero was shifted back toward the old glowing-orb vibe
...
- `public/assets/hermeschiworld/characters/README.md`

What this means:
- canonical asset path now exists for believable humanoid GLBs
- canonical path:
  - `/public/assets/hermeschiworld/characters/<id>.glb`
- legacy `/avatars-3d/<id>.glb` remains as fallback

### 10. Execution docs/specs were created
Created / updated:
- `docs/hermeschiworld/landing-page-spec.md`
- `docs/hermeschiworld/graphics-usability-plan.md`
- `docs/hermeschiworld/agora-believable-checklist.md`
...
1. **Turned HermesChiWorld landing into a real standalone launch page** instead of a workspace-fragment/app-pane.
2. **Removed or traced most of the workspace chrome leaks** that kept showing up in HermesChiWorld surfaces.
...
### A. Final HermesChiWorld landing cleanup
Still verify/fix:
- no remaining root overlay leaks on `/hermeschi-world` or `/playground`
- no bottom-right usage/session pill anywhere on HermesChiWorld/game surfaces
- no mobile-access prompt / other workspace popups on HermesChiWorld routes
- no sidebar overlap edge cases on `/playground`

### B. Re-verify local previews cleanly
Need a fresh sanity pass on:
- `http://127.0.0.1:3002/hermeschi-world`
...
- `public/assets/hermeschiworld/characters/*`
