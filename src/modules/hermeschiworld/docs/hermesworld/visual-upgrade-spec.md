# HermesChiWorld Visual Upgrade Spec

Status: draft locked for implementation
Owner: Eric / Aurora
Target release train: post-v2.2.x, candidate for v2.3.0 polish wave
Reference inputs: TinySkies, current HermesChiWorld build, internal screenshots, multiplayer/MMO readability goals

## 1. Goal

Upgrade HermesChiWorld from "strong prototype / shippable novelty" to "memorable stylized game surface" without changing the core product concept.

The point is **not** to turn HermesChiWorld into a different game.
The point is to:
...
- no dependency on a separate HermesChiWorld repo
...
1. HermesChiWorld lives **inside HermesChi**
...
HermesChiWorld visual pass is successful if:
