...
export function getUnavailableReason(
  ...
  const probes = normalized
    ...
    : '/api/gateway-status'
  return `${getFeatureLabel(feature)} is not reachable through the local HermesChi probes yet. Verify ${probes} before starting another gateway; if those endpoints pass, refresh or reprobe the Workspace UI.`
}
