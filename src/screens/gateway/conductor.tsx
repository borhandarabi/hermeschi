...
const QUICK_ACTIONS: Array<{
  ...
  {
    id: 'research',
    label: t('conductor.quickAction.research'),
    icon: Search01Icon,
    prompt: t('conductor.quickAction.researchPrompt'),
  },
  {
    id: 'build',
    label: t('conductor.quickAction.build'),
    icon: PlayIcon,
    prompt: t('conductor.quickAction.buildPrompt'),
  },
  {
    id: 'review',
    label: t('conductor.quickAction.review'),
    icon: TaskDone01Icon,
    prompt: t('conductor.quickAction.reviewPrompt'),
  },
  {
    id: 'deploy',
    label: t('conductor.quickAction.deploy'),
    icon: Rocket01Icon,
    prompt: t('conductor.quickAction.deployPrompt'),
  },
]
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
      <button type="button" onClick={onToggle} aria-expanded={expanded} className="flex w-full items-start justify-between gap-4 text-left">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.cost.title')}</p>
          <p className="mt-1 text-sm text-[var(--theme-muted-2)]">{t('conductor.cost.desc')}</p>
        </div>
        <span className="inline-flex items-center gap-2 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-2 text-xs font-medium text-[var(--theme-text)]">
          {expanded ? t('conductor.cost.hide') : t('conductor.cost.show')}
        ...
        <div className="mt-4 space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--theme-muted)]">{t('conductor.cost.totalTokens')}</p>
              <p className="mt-2 text-2xl font-semibold text-[var(--theme-text)]">{totalTokens.toLocaleString()}</p>
            </div>
            <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--theme-muted)]">{t('conductor.cost.estimatedCost')}</p>
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)]">
            <div className="flex items-center justify-between border-b border-[var(--theme-border)] px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--theme-muted)]">
              <span>{t('conductor.cost.workers')}</span>
              <span>{t('conductor.cost.costColumn')}</span>
            </div>
            {workers.length > 0 ? (
              <div className="divide-y divide-[var(--theme-border)]">
                {workers.map((worker) => (
                  <div key={worker.id} className="flex items-center gap-3 px-4 py-3 text-sm">
                    ...
                    <span className="text-xs text-[var(--theme-muted)]">{worker.totalTokens.toLocaleString()} {t('conductor.cost.tok')}</span>
              ...
              <div className="px-4 py-3 text-sm text-[var(--theme-muted)]">{t('conductor.cost.noWorkersDetail')}</div>
            )}
          </div>
        </div>
      ) : null}
...
const PLANNING_STEPS = [
  t('conductor.planning.step1'),
  t('conductor.planning.step2'),
  t('conductor.planning.step3'),
  t('conductor.planning.step4'),
]
const WORKING_STEPS = [
  t('conductor.working.step1'),
  t('conductor.working.step2'),
  t('conductor.working.step3'),
  t('conductor.working.step4'),
  t('conductor.working.step5'),
  t('conductor.working.step6'),
  t('conductor.working.step7'),
  t('conductor.working.step8'),
  t('conductor.working.step9'),
]
  ...
  if (isPaused) {
    return (
      <div className="flex items-center gap-3 py-3">
        ...
        <p className="text-sm text-[var(--theme-muted)]">{t('conductor.status.paused')}</p>
      </div>
    )
  }
...
function getOutputDisplayName(projectPath: string | null | undefined): string {
  if (!projectPath) return t('conductor.outputReady')
  return projectPath.split('/').pop() || t('conductor.outputIndexHtml')
}
...
function formatElapsedTime(startIso: string | null | undefined, now: number): string {
  if (!startIso) return t('conductor.time.zero')
  const startMs = new Date(startIso).getTime()
  if (!Number.isFinite(startMs)) return t('conductor.time.zero')
...
function formatElapsedMilliseconds(durationMs: number): string {
  ...
  if (hours > 0) return t('conductor.time.elapsedHms', { hours, minutes, seconds })
  if (minutes > 0) return t('conductor.time.elapsedMs', { minutes, seconds })
  return t('conductor.time.elapsedS', { seconds })
}
...
function formatRelativeTime(value: string | null | undefined, now: number): string {
  if (!value) return t('conductor.time.justNow')
  const ms = new Date(value).getTime()
  if (!Number.isFinite(ms)) return t('conductor.time.justNow')
  const diffSeconds = Math.max(0, Math.floor((now - ms) / 1000))
  if (diffSeconds < 10) return t('conductor.time.justNow')
  if (diffSeconds < 60) return t('conductor.time.secondsAgo', { count: diffSeconds })
  const diffMinutes = Math.floor(diffSeconds / 60)
  if (diffMinutes < 60) return t('conductor.time.minutesAgo', { count: diffMinutes })
  const diffHours = Math.floor(diffMinutes / 60)
  return t('conductor.time.hoursAgo', { count: diffHours })
}
...
function getWorkerDot(status: 'running' | 'complete' | 'stale' | 'idle') {
  if (status === 'complete') return { dotClass: 'bg-emerald-400', label: t('conductor.status.complete') }
  if (status === 'running') return { dotClass: 'bg-sky-400 animate-pulse', label: t('conductor.status.running') }
  if (status === 'idle') return { dotClass: 'bg-amber-400', label: t('conductor.status.idle') }
  return { dotClass: 'bg-red-400', label: t('conductor.status.stale') }
}
    ...
    <div key={worker.key} className={cn('overflow-hidden rounded-2xl border border-[var(--theme-border)] border-l-4 bg-[var(--theme-card)] px-4 py-3', getWorkerBorderClass(worker.status))}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        <div className="rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-2">
          <p className="text-[var(--theme-muted)]">{t('conductor.worker.model')}</p>
          <p className="mt-1 truncate text-[var(--theme-text)]">{getShortModelName(worker.model)}</p>
        </div>
        <div className="rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-2">
          <p className="text-[var(--theme-muted)]">{t('conductor.worker.tokens')}</p>
          <p className="mt-1 text-[var(--theme-text)]">{worker.tokenUsageLabel}</p>
        </div>
        <div className="rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-2">
          <p className="text-[var(--theme-muted)]">{t('conductor.worker.elapsed')}</p>
          <p className="mt-1 text-[var(--theme-text)]">{formatElapsedTime(workerStartedAt, workerEndTime)}</p>
        </div>
        <div className="rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-2">
          <p className="text-[var(--theme-muted)]">{t('conductor.worker.lastUpdate')}</p>
        ...
        {open ? (
          <div className="absolute left-0 top-[calc(100%+0.5rem)] z-[80] w-full overflow-hidden rounded-2xl border border-[var(--theme-border2)] bg-[var(--theme-card)] shadow-[0_24px_80px_var(--theme-shadow)]">
            <div className="max-h-80 overflow-y-auto p-2">
              ...
              >
                <span className={cn('size-2 rounded-full', !value ? 'bg-[var(--theme-accent)]' : 'bg-[var(--theme-border2)]')} />
                <span className="min-w-0 flex-1 truncate">{t('conductor.worker.defaultAuto')}</span>
                <span className="rounded-full border border-[var(--theme-border)] bg-[var(--theme-card2)] px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[var(--theme-muted)]">{t('conductor.worker.auto')}</span>
              </button>
...
export function Conductor() {
  ...
  useEffect(() => {
    ...
    const loadDirectory = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(data.error || t('conductor.modal.directoryLoadFailed'))
        }
        ...
        setDirectoryBrowserError(error instanceof Error ? error.message : t('conductor.modal.directoryLoadFailed'))
      } finally {
        if (!cancelled) {
  ...
  const directoryBreadcrumbs = useMemo(() => {
    ...
    return segments.map((segment, index) => ({
      label: segment === '/' ? t('conductor.modal.rootLabel') : segment,
  ...
  const homeOfficeRows = useMemo<AgentWorkingRow[]>(() => {
    ...
    if (sessions.length === 0) {
      return OFFICE_NAMES.slice(0, 3).map((name, i) => ({
        ...
        lastLine: t('conductor.worker.waitingForWork'),
        taskCount: 0,
        roleDescription: t('conductor.worker.roleWorker'),
      }))
    }
    return sessions.slice(0, 6).map((session, i) => {
      ...
      return {
        ...
        status,
        lastLine: s.task ?? s.label ?? s.title ?? s.derivedTitle ?? t('conductor.worker.workingFallback'),
        lastAt: updatedAt || undefined,
        taskCount: 0,
        roleDescription: s.label ?? t('conductor.worker.roleWorker'),
    ...
    if (conductor.workers.length > 0) {
      return conductor.workers.map((worker, index) => {
        ...
        return {
          ...
          lastLine: isWorkerPaused ? t('conductor.status.paused') : lastLine,
          ...
          currentTask: isWorkerPaused ? t('conductor.status.paused') : currentTask,
    ...
    return [
      {
        ...
        roleDescription: t('conductor.worker.roleWaitingForWorkers'),
        status: 'spawning',
        lastLine: conductor.goal || t('conductor.worker.preparingOffice'),
        taskCount: 0,
        currentTask: conductor.goal || t('conductor.worker.preparingOffice'),
  ...
  const completeSummary = useMemo(() => {
    ...
    const lines = [
      isFailed ? t('conductor.summary.missionFailed', { error: conductor.streamError ?? '' }) : t('conductor.summary.missionCompleted'),
      '',
      t('conductor.summary.goal', { goal: conductor.goal }),
      t('conductor.summary.duration', { duration: formatElapsedTime(conductor.missionStartedAt, conductor.completedAt ? new Date(conductor.completedAt).getTime() : now) }),
    ]
    if (totalWorkers > 0) {
      lines.push(t('conductor.summary.workers', { count: totalWorkers, tokens: totalTokens.toLocaleString() }))
    }
    if (completePhaseProjectPath) {
      lines.push(t('conductor.summary.output', { label: completePhaseOutputLabel }))
    }
    return lines.join('\n')
  ...
  if (phase === 'home') {
    if (selectedHistoryEntry) {
      ...
      const historyStatusLabel = selectedHistoryEntry.status === 'completed' ? t('conductor.status.complete') : t('conductor.history.stopped')
        ...
        <div className="flex min-h-dvh flex-col overflow-y-auto bg-[var(--theme-bg)] text-[var(--theme-text)]" style={THEME_STYLE}>
          <main className="mx-auto flex min-h-0 w-full max-w-[720px] flex-1 flex-col px-4 py-4 pb-4 md:pb-[calc(var(--tabbar-h,80px)+1rem)] md:px-6 md:py-8">
            <div className="space-y-6">
              ...
              >
                <span aria-hidden="true">←</span> {t('conductor.history.back')}
              </button>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className={cn('text-xs font-semibold uppercase tracking-[0.24em]', selectedHistoryEntry.status === 'completed' ? 'text-[var(--theme-accent)]' : 'text-red-400')}>
                      {selectedHistoryEntry.status === 'completed' ? t('conductor.history.missionComplete') : t('conductor.history.missionStopped')}
                    </p>
                    ...
                    <p className="mt-2 text-xs text-[var(--theme-muted-2)]">
                      {t('conductor.history.workersFinished', { completed: selectedHistoryEntry.workerCount, total: Math.max(selectedHistoryEntry.workerCount, 1) })} ·{' '}
                      {t('conductor.history.totalElapsed', { time: formatDurationRange(selectedHistoryEntry.startedAt, selectedHistoryEntry.completedAt, now) })}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    >
                      {t('conductor.history.newMission')}
                    </Button>
                  </div>
                </div>
              </div>
              ...
              {selectedHistoryOutputPath && selectedHistoryPreview.ready ? (
                <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.history.outputPreview')}</p>
                    ...
                    >
                      {t('conductor.history.openInNewTab')}
                    </a>
                  </div>
                  <div className="mt-4 overflow-auto rounded-2xl border border-[var(--theme-border)] bg-white">
                    <iframe src={selectedHistoryPreviewUrl!} className="h-[clamp(280px,55vh,520px)] w-full" sandbox="allow-scripts allow-same-origin" title={t('conductor.history.titleHistoryPreview')} />
                  </div>
                </section>
                ...
                <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                  <div className="flex items-center gap-3 text-sm text-[var(--theme-muted)]">
                    ...
                    {t('conductor.history.loadingPreview')}
                  </div>
                </section>
              ) : selectedHistoryOutputPath && selectedHistoryPreview.unavailable ? (
                showHistoryOutputFallback ? null : (
                  <p className="px-1 text-sm text-[var(--theme-muted)]">{t('conductor.history.noPreviewAvailable')}</p>
                )
              ) : null}
              ...
              <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.history.agentSummary')}</p>
                  </div>
                ...
                <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
                  {historySummary ? (
                    ...
                    <p className="text-sm text-[var(--theme-muted)]">{t('conductor.history.noSummary')}</p>
                  )}
                </div>
                    ...
                    {historyWorkerDetails.map((worker: MissionHistoryWorkerDetail, index) => (
                      <div key={`${selectedHistoryEntry.id}-worker-${index}`} className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm">
                        ...
                        <span className="ml-auto text-xs text-[var(--theme-muted)]">
                          {getShortModelName(worker.model)} · {worker.totalTokens.toLocaleString()} {t('conductor.cost.tok')}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                {selectedHistoryEntry.streamText && selectedHistoryEntry.completeSummary && (
                  <details className="mt-4 overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
                    <summary className="cursor-pointer text-xs font-medium text-[var(--theme-muted)]">{t('conductor.history.rawAgentOutput')}</summary>
              ...
              {showHistoryOutputFallback ? (
                <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.history.output')}</p>
                      <p className="mt-1 text-xs text-[var(--theme-muted-2)]">
                        {selectedHistoryOutputPath ? t('conductor.history.previewUnavailableFor', { label: selectedHistoryOutputLabel }) : t('conductor.history.previewUnavailable')}
                      </p>
                    </div>
                  </div>
                  <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
                ...
                <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.history.workerOutput')}</p>
                  <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
                    <Markdown className="max-h-[600px] max-w-none overflow-auto text-sm text-[var(--theme-text)]">{historyOutputText}</Markdown>
                  </div>
                </section>
              ...
              {!historySummary && historyWorkerDetails.length === 0 && !selectedHistoryOutputPath && !selectedHistoryEntry.workerSummary?.length && !historyOutputText && (
                <section className="overflow-hidden rounded-3xl border border-dashed border-[var(--theme-border)] bg-[var(--theme-card)] p-6">
                  <p className="text-center text-sm text-[var(--theme-muted)]">
                    {t('conductor.history.noDetailedOutput')}
                    <br />
                    <span className="text-xs text-[var(--theme-muted-2)]">{t('conductor.history.futureMissionsNote')}</span>
                  </p>
                </section>
              )}
            </div>
            ...
            <div className="space-y-2 md:text-center">
              <div className="flex items-center gap-2">
                ...
                <div className="hidden md:inline-flex shrink-0 items-center gap-2.5 rounded-full border border-[var(--theme-border)] bg-[var(--theme-card)] px-5 py-2.5 text-sm font-semibold uppercase tracking-[0.24em] text-[var(--theme-muted)]">
                  <span>{t('conductor.home.eyebrow')}</span>
                ...
                <div className="flex md:flex-1 items-center justify-end gap-2 ml-auto md:ml-0">
                  <WorkflowHelpModal
                    compact
                    eyebrow={t('conductor.home.eyebrow')}
                    title={t('conductor.home.title')}
                    sections={[
                      {
                        title: t('conductor.home.whatConductorIsFor'),
                        bullets: [
                          t('conductor.home.bullet1'),
                          t('conductor.home.bullet2'),
                        ],
                      },
                      {
                        title: t('conductor.home.typicalFlow'),
                        bullets: [
                          t('conductor.home.flowBullet1'),
                          t('conductor.home.flowBullet2'),
                        ],
                      },
                      {
                        title: t('conductor.home.faq'),
                        bullets: [
                          t('conductor.home.faqBullet1'),
                          t('conductor.home.faqBullet2'),
                        ],
                      },
                    ]}
                  />
                  <button
                    ...
                    aria-label={t('conductor.home.ariaNewMission')}
                  >
                  ...
                  <button
                    ...
                    aria-label={t('conductor.home.ariaOpenSettings')}
                  >
                    <HugeiconsIcon icon={Settings01Icon} size={18} strokeWidth={1.7} />
                  </button>
                </div>
              </div>
              <p className="text-sm text-[var(--theme-muted-2)]">{t('conductor.home.launchDesc')}</p>
            </div>
            ...
            {hasMissionHistory || conductor.recentSessions.length > 0 ? (
              <section className="mt-6 w-full space-y-3">
                <div className="flex items-center gap-3">
                  <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--theme-muted)]">{t('conductor.home.recentMissions')}</h2>
                ...
                <div className="flex items-center gap-1">
                  {(['all', 'completed', 'failed'] as const).map((filter) => (
                    ...
                    >
                      {filter === 'all' ? t('conductor.home.filterAll') : filter === 'completed' ? t('conductor.home.filterCompleted') : t('conductor.home.filterFailed')}
                    </button>
                  ))}
                </div>
                {visibleActivityItems.length > 0 ? (
                      ...
                      ? visibleActivityItems.map((item) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                {entry.status === 'completed' ? t('conductor.status.complete') : t('conductor.status.failed')}
                              </span>
                              <span className="w-[48px] shrink-0 text-right text-xs text-[var(--theme-muted-2)]">{formatRelativeTime(entry.completedAt, now)}</span>
                              <span className="hidden shrink-0 text-right text-xs text-[var(--theme-muted)] sm:inline">{entry.totalTokens.toLocaleString()} {t('conductor.cost.tok')}</span>
                            </button>
                          )
                        })
                      : visibleActivityItems.map((item) => {
                          ...
                          return (
                            <div key={recentSession.key} className="flex items-center gap-2 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card)] px-3 py-2 text-sm sm:gap-3">
                              ...
                              >
                                ...
                                {sessionStatus === 'completed' ? t('conductor.status.complete') : sessionStatus === 'failed' ? t('conductor.status.failed') : t('conductor.status.running')}
                              </span>
                              <span className="shrink-0 text-xs text-[var(--theme-muted-2)]">{formatRelativeTime(updatedAt, now)}</span>
                              <span className="hidden shrink-0 text-xs text-[var(--theme-muted)] sm:inline">{tokens.toLocaleString()} {t('conductor.cost.tok')}</span>
                  ...
                  <div className="rounded-xl border border-dashed border-[var(--theme-border)] px-4 py-6 text-center text-sm text-[var(--theme-muted)]">
                    {t('conductor.home.noItemsFound', {
                      filter: activityFilter === 'all' ? t('conductor.home.filterAllPrefix') : activityFilter === 'completed' ? t('conductor.home.filterCompletedPrefix') : t('conductor.home.filterFailedPrefix'),
                      kind: hasMissionHistory ? t('conductor.home.missionsKind') : t('conductor.home.sessionsKind'),
                    })}
                  </div>
                )}
              </section>
              ...
              <section className="mt-6 w-full">
                <div className="rounded-xl border border-dashed border-[var(--theme-border)] px-4 py-8 text-center">
                  <p className="text-sm text-[var(--theme-muted)]">{t('conductor.home.noMissions')}</p>
                  <p className="mt-1 text-xs text-[var(--theme-muted-2)]">{t('conductor.home.launchFirstMission')}</p>
                </div>
              </section>
            )}
          </div>
            ...
            >
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-lg font-semibold tracking-tight text-[var(--theme-text)]">{t('conductor.modal.newMission')}</h2>
                    <p className="mt-1 text-sm text-[var(--theme-muted-2)]">{t('conductor.modal.newMissionDesc')}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setMissionModalOpen(false)}
                    ...
                    aria-label={t('conductor.modal.ariaCloseNewMission')}
                  >
                    ×
                  </button>
                </div>
                ...
                >
                  ...
                  <textarea
                    ...
                    placeholder={t('conductor.modal.placeholderPrefix', { label: QUICK_ACTIONS.find((action) => action.id === selectedAction)?.label ?? t('conductor.quickAction.build') })}
                  ...
                  <div className="flex justify-end">
                    <Button type="submit" disabled={!goalDraft.trim() || conductor.isSending} className="rounded-full bg-[var(--theme-accent)] px-5 text-white hover:bg-[var(--theme-accent-strong)]">
                      {conductor.isSending ? t('conductor.modal.launching') : t('conductor.modal.launchMission')}
          ...
          {settingsOpen && (
            ...
            >
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.modal.missionDefaults')}</p>
                    <h2 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--theme-text)]">{t('conductor.modal.settingsTitle')}</h2>
                    <p className="mt-2 text-sm text-[var(--theme-muted-2)]">{t('conductor.modal.settingsDesc')}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setSettingsOpen(false)}
                    ...
                    aria-label={t('conductor.modal.ariaCloseSettings')}
                  >
                    ×
                  </button>
                </div>
                ...
                <div className="mt-6 space-y-4">
                  <ModelSelectorDropdown
                    label={t('conductor.modal.orchestratorModel')}
                  ...
                  <ModelSelectorDropdown
                    label={t('conductor.modal.workerModel')}
                  ...
                  <div className="space-y-2">
                    <span className="text-sm font-medium text-[var(--theme-text)]">{t('conductor.modal.projectDirectory')}</span>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        value={conductor.conductorSettings.projectsDir}
                        onChange={(event) => updateSettings({ projectsDir: event.target.value })}
                        placeholder={t('conductor.modal.projectDirPlaceholder')}
                      ...
                      >
                        {t('conductor.modal.browse')}
                      </button>
                    </div>
                    <p className="text-xs text-[var(--theme-muted-2)]">{t('conductor.modal.projectDirHint')}</p>
                  </div>

                  <label className="block space-y-2">
                    <span className="text-sm font-medium text-[var(--theme-text)]">{t('conductor.modal.maxParallelWorkers')}</span>
                    <input
                      type="number"
                      min={1}
                      max={5}
                  ...
                  <label className="flex items-start gap-3 rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-4 py-4">
                    ...
                    <span className="min-w-0">
                      <span className="block text-sm font-medium text-[var(--theme-text)]">{t('conductor.modal.supervisedMode')}</span>
                      <span className="mt-1 block text-sm text-[var(--theme-muted-2)]">{t('conductor.modal.supervisedDesc')}</span>
                    </span>
                  </label>
                  ...
                  {canResetSavedState ? (
                    <div className="flex items-center justify-between rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-4 py-3">
                      <div>
                        <p className="text-sm font-medium text-[var(--theme-text)]">{t('conductor.modal.resetSavedState')}</p>
                        <p className="mt-1 text-xs text-[var(--theme-muted-2)]">{t('conductor.modal.resetSavedStateDesc')}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => {
          ...
          {directoryBrowserOpen ? (
            <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[color-mix(in_srgb,var(--theme-bg)_55%,transparent)] px-4 py-6 backdrop-blur-md" onClick={closeDirectoryBrowser}>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.modal.directoryBrowser')}</p>
                    <h3 className="mt-2 text-xl font-semibold tracking-tight text-[var(--theme-text)]">{t('conductor.modal.chooseProjectDir')}</h3>
                    <p className="mt-2 text-sm text-[var(--theme-muted-2)]">{t('conductor.modal.chooseProjectDirDesc')}</p>
                  </div>
                  <button
                    type="button"
                    onClick={closeDirectoryBrowser}
                    className="inline-flex size-10 items-center justify-center rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] text-lg text-[var(--theme-muted)] transition-colors hover:border-[var(--theme-accent)] hover:text-[var(--theme-accent-strong)]"
                    aria-label={t('conductor.modal.ariaCloseDirectoryBrowser')}
                  >
                    ×
                  </button>
                </div>
                ...
                <div className="mt-5 space-y-4">
                  ...
                  <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-4 py-3">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--theme-muted)]">{t('conductor.modal.currentPath')}</span>
                  ...
                  <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)]">
                    <div className="flex items-center justify-between border-b border-[var(--theme-border)] px-4 py-3">
                      <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--theme-muted)]">{t('conductor.modal.folders')}</span>
                      {directoryBrowserLoading ? (
                        <span className="text-xs text-[var(--theme-muted-2)]">{t('conductor.modal.loadingShort')}</span>
                      ) : (
                        <span className="text-xs text-[var(--theme-muted-2)]">{t('conductor.modal.folderCount', { count: directoryBrowserEntries.length })}</span>
                      )}
                    </div>
                    <div className="max-h-[22rem] overflow-y-auto p-2">
                      {directoryBrowserLoading ? (
                        <div className="flex items-center justify-center gap-3 px-4 py-10 text-sm text-[var(--theme-muted)]">
                          <div className="size-4 animate-spin rounded-full border-2 border-[var(--theme-border)] border-t-[var(--theme-accent)]" />
                          <span>{t('conductor.modal.loadingFolders')}</span>
                        </div>
                        ...
                        <div className="space-y-1">
                          {directoryBrowserEntries.map((entry) => (
                            ...
                            >
                              ...
                              <span className="text-xs text-[var(--theme-muted)]">{t('conductor.modal.open')}</span>
                            </button>
                          ))}
                        </div>
                      ) : (
                        <div className="px-4 py-10 text-center text-sm text-[var(--theme-muted)]">{t('conductor.modal.noFoldersFound')}</div>
                      )}
                    </div>
                  </div>

                  <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-4 py-3">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--theme-muted)]">{t('conductor.modal.quickPaths')}</p>
                  ...
                  <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                    ...
                    >
                      {t('conductor.modal.selectThisDirectory')}
                    </button>
                  </div>
                </div>
              </div>
        ...
        <main className="mx-auto flex min-h-0 w-full max-w-[720px] flex-1 flex-col px-4 py-4 pb-4 md:pb-[calc(var(--tabbar-h,80px)+1rem)] md:px-6 md:py-8">
          <div className="space-y-6">
            <div className="text-center">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--theme-accent)]">{t('conductor.preview.missionDecomposition')}</p>
              <h1 className="text-2xl font-semibold tracking-tight">{conductor.goal}</h1>
              <p className="text-sm text-[var(--theme-muted-2)]">{t('conductor.preview.decompositionDesc')}</p>
            </div>
            ...
            <section className="rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
              <div className="flex items-center justify-between gap-3 border-b border-[var(--theme-border)] pb-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.preview.missionPlanning')}</p>
                  <p className="mt-1 text-xs text-[var(--theme-muted-2)]">{t('conductor.preview.analyzingRequest')}</p>
                </div>
                <span className="rounded-full border border-sky-400/30 bg-sky-500/10 px-3 py-1 text-xs font-medium text-sky-300 animate-pulse">{t('conductor.preview.working')}</span>
              </div>
              ...
              {conductor.timeoutWarning && (
                <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-amber-400/40 bg-amber-500/10 px-5 py-3">
                  <p className="text-sm text-amber-700">{t('conductor.preview.planningTakingLong')}</p>
                  <Button
                    type="button"
                  ...
                  >
                    {t('conductor.preview.cancel')}
                  </Button>
                </div>
              )}
              {conductor.tasks.length > 0 && (
                <div className="mt-4 space-y-2">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.preview.identifiedTasks', { count: conductor.tasks.length })}</p>
          ...
          <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-[var(--theme-danger-border)] bg-[var(--theme-danger-soft)] px-5 py-4">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-[var(--theme-danger)]">{t('conductor.complete.missionFailed')}</p>
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    ...
                    >
                      {t('conductor.complete.retryMission')}
                    </Button>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className={cn('text-xs font-semibold uppercase tracking-[0.24em]', conductor.streamError ? 'text-red-400' : 'text-[var(--theme-accent)]')}>
                    {conductor.streamError ? t('conductor.complete.missionStopped') : t('conductor.complete.missionComplete')}
                  </p>
                  <h1 className="mt-2 text-xl font-semibold tracking-tight text-[var(--theme-text)] sm:text-2xl">{conductor.goal}</h1>
                  <p className="mt-2 text-xs text-[var(--theme-muted-2)]">
                    {t('conductor.complete.workersFinished', { completed: completedWorkers, total: Math.max(totalWorkers, completedWorkers) })} ·{' '}
                    {t('conductor.complete.totalElapsed', { time: formatElapsedTime(conductor.missionStartedAt, conductor.completedAt ? new Date(conductor.completedAt).getTime() : now) })}
                  </p>
                </div>
                <div className="flex gap-2">
            ...
            {completePhaseProjectPath && previewState.ready ? (
              <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.history.outputPreview')}</p>
                    <p className="mt-1 text-xs text-[var(--theme-muted-2)]">{completePhaseProjectPath.split('/').pop() || t('conductor.outputIndexHtml')}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <a
                      href={previewUrl!}
                      ...
                      className="inline-flex items-center gap-2 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] px-3 py-1.5 text-xs font-medium text-[var(--theme-text)] transition-colors hover:border-[var(--theme-accent)] hover:text-[var(--theme-accent)]"
                    >
                      {t('conductor.complete.openInNewTab')}
                    </a>
                    <button
                      type="button"
                      onClick={() => setContinueModalOpen(true)}
                ...
                <div className="mt-4 overflow-auto rounded-2xl border border-[var(--theme-border)] bg-white">
                  <iframe src={previewUrl!} className="h-[clamp(280px,55vh,520px)] w-full" sandbox="allow-scripts allow-same-origin" title={t('conductor.history.titleMissionOutputPreview')} />
                </div>
              </section>
              ...
              <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                <div className="flex items-center gap-3 text-sm text-[var(--theme-muted)]">
                  <div className="size-4 animate-spin rounded-full border-2 border-[var(--theme-border)] border-t-[var(--theme-accent)]" />
                  {t('conductor.complete.loadingPreview')}
                </div>
              </section>
            ) : null}
              ...
              (() => {
                ...
                return (
                  <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.complete.output')}</p>
                        <p className="mt-1 text-xs text-[var(--theme-muted-2)]">{completePhaseProjectPath ? t('conductor.complete.previewUnavailableFor', { label: completePhaseOutputLabel }) : t('conductor.complete.agentWorkOutput')}</p>
                      </div>
                    </div>
                    <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
            ...
            {conductor.tasks.length > 1 && completedTaskOutputs.length > 0 && (
              <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.complete.taskOutputs')}</p>
                    <p className="mt-1 text-xs text-[var(--theme-muted-2)]">{t('conductor.complete.taskOutputsDesc')}</p>
                  </div>
                </div>
            ...
            <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-6 shadow-[0_24px_80px_var(--theme-shadow)]">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">{t('conductor.complete.agentSummary')}</p>
                </div>
                <span
                  className={cn(
                ...
                >
                  {conductor.streamError ? t('conductor.status.stopped') : t('conductor.status.complete')}
                </span>
              </div>
              <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
                {completeSummary ? (
                  ...
                  <p className="text-sm text-[var(--theme-muted)]">{t('conductor.complete.noSummary')}</p>
                )}
              </div>
                  ...
                  {conductor.workers.map((worker, index) => {
                    ...
                    return (
                      <div key={worker.key} className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm">
                        ...
                        <span className="text-[var(--theme-muted)]">{worker.label}</span>
                        <span className="ml-auto text-xs text-[var(--theme-muted)]">
                          {shortModelName} · {worker.totalTokens.toLocaleString()} {t('conductor.cost.tok')}
                        </span>
                      </div>
                    )
                  })}
              ...
              {conductor.streamText && completeSummary && (
                <details className="mt-4 overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-5 py-4">
                  <summary className="cursor-pointer text-xs font-medium text-[var(--theme-muted)]">{t('conductor.complete.rawAgentOutput')}</summary>
                  <div className="mt-4 border-t border-[var(--theme-border)] pt-4">
                    <Markdown className="max-h-[400px] max-w-none overflow-auto text-sm text-[var(--theme-text)]">{conductor.streamText}</Markdown>
                  </div>
                </details>
          ...
          {continueModalOpen ? (
            ...
            >
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-lg font-semibold tracking-tight text-[var(--theme-text)]">{t('conductor.complete.continueMission')}</h2>
                  </div>
                  <button
                    type="button"
                    onClick={() => setContinueModalOpen(false)}
                    className="inline-flex size-9 items-center justify-center rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card2)] text-lg text-[var(--theme-muted)] transition-colors hover:border-[var(--theme-accent)] hover:text-[var(--theme-accent-strong)]"
                    aria-label={t('conductor.complete.ariaCloseContinue')}
                  >
                    ×
                  </button>
                </div>
                ...
                {continuationModalPreview ? (
                  <div className="mt-4 rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg)] px-4 py-3">
                    <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--theme-muted)]">{t('conductor.complete.previousOutputSummary')}</p>
                ...
                >
                  <input
                    ...
                    placeholder={t('conductor.complete.continuePlaceholder')}
                    disabled={conductor.isSending}
                  ...
                  <div className="flex justify-end">
                    ...
                    >
                      ...
                      {conductor.isSending ? t('conductor.complete.sending') : t('conductor.complete.send')}
                    </button>
                  </div>
                </form>
              </div>
      ...
      <main className="mx-auto flex min-h-0 w-full max-w-[720px] flex-1 flex-col px-4 py-4 pb-4 md:pb-[calc(var(--tabbar-h,80px)+1rem)] md:px-6 md:py-8">
        <div className="flex w-full flex-col gap-6">
          ...
          <section className="overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] px-5 py-5 shadow-[0_24px_80px_var(--theme-shadow)]">
            <div className="text-center">
              ...
              <div className="mt-2 flex items-center justify-center gap-2 text-xs text-[var(--theme-muted)]">
                ...
                <span>
                  {t('conductor.active.workersComplete', { completed: completedWorkers, total: Math.max(totalWorkers, 1) })}
                </span>
                <span className="text-[var(--theme-border)]">·</span>
                <span>{t('conductor.active.activeWorkers', { count: activeWorkerCount })}</span>
              </div>
              {conductor.isPaused ? (
            ...
            <div className="mt-3 flex items-center justify-center gap-2">
              ...
              >
                <span>■</span> {t('conductor.active.stopMission')}
              </button>
              <button
                type="button"
              ...
              >
                <span>{conductor.isPaused ? '▶' : '⏸'}</span> {conductor.isPausing ? '...' : conductor.isPaused ? t('conductor.active.resume') : t('conductor.active.pause')}
              </button>
            </div>
          </section>
          {conductor.timeoutWarning && (
            <section className="rounded-2xl border border-[var(--theme-warning-border)] bg-[var(--theme-warning-soft)] px-5 py-4">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-semibold text-[var(--theme-warning)]">{t('conductor.active.stalledTitle')}</p>
                  <p className="mt-1 text-xs text-[var(--theme-muted)]">{t('conductor.active.stalledDesc')}</p>
                </div>
                <div className="flex flex-col gap-2 sm:flex-row">
                  <Button
                    type="button"
                  ...
                  >
                    {t('conductor.active.keepWaiting')}
                  </Button>
                  <Button
                    type="button"
                    onClick={() => void conductor.stopMission()}
                  ...
                  >
                    {t('conductor.active.stopMission')}
                  </Button>
                </div>
              </div>
            </section>
          ...
          <section className="max-h-[clamp(200px,40vh,360px)] overflow-hidden rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-card)] shadow-[0_24px_80px_var(--theme-shadow)]">
            <OfficeView agentRows={officeAgentRows} missionRunning onViewOutput={() => {}} processType="parallel" companyName={t('conductor.active.companyName')} containerHeight={360} hideHeader />
          </section>
            ...
            <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
              <div className="space-y-2">
                <h2 className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--theme-muted)]">
                  {t('conductor.active.tasks', { completed: conductor.tasks.filter((task) => task.status === 'complete').length, total: conductor.tasks.length })}
                </h2>
                {conductor.tasks.map((task) => {
              ...
              <div className="space-y-3">
                {selectedTaskId ? (
                  <div className="flex items-center justify-between gap-3">
                    <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--theme-muted)]">{t('conductor.active.taskOutput')}</h2>
                  </div>
                ) : null}
                {(() => {
                        ...
                        <div className="rounded-2xl border border-dashed border-[var(--theme-border)] bg-[var(--theme-card)] px-4 py-8 text-center text-sm text-[var(--theme-muted)] md:col-span-2">
                          <div className="flex flex-col items-center gap-2">
                            <div className="flex items-center justify-center gap-3">
                              ...
                              <span>{t('conductor.active.spawningWorkers')}</span>
                            </div>
