...
# Install HermesChi as a user-level service.
# macOS: launchd user agent
# Linux: systemd --user unit

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SERVICE_NAME="hermeschi"
PORT="${PORT:-3000}"
HOST="${HOST:-127.0.0.1}"
...
if [[ "${1:-install}" == "uninstall" ]]; then
  case "$(uname -s)" in
    Darwin)
      plist="$HOME/Library/LaunchAgents/com.hermeschi.workspace.plist"
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.hermeschi.workspace</string>
  ...
  <key>StandardOutPath</key><string>$ROOT_DIR/logs/hermeschi.out.log</string>
  <key>StandardErrorPath</key><string>$ROOT_DIR/logs/hermeschi.err.log</string>
</dict>
</plist>
EOF
    launchctl bootout "gui/$(id -u)" "$plist" 2>/dev/null || true
  ...
  Linux)
    ...
    cat > "$unit" <<EOF
[Unit]
Description=HermesChi dashboard
