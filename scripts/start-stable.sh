...
PID_FILE="$RUNTIME_DIR/hermeschi.pid"
LOG_FILE="$RUNTIME_DIR/hermeschi.log"
BUILD_LOG_FILE="$RUNTIME_DIR/hermeschi.build.log"
mkdir -p "$RUNTIME_DIR"
...
echo "[stable] building HermesChi..."
rm -rf dist
pnpm build >"$BUILD_LOG_FILE" 2>&1

echo "[stable] starting HermesChi on port $PORT..."
